...
public class ArgumentDependency extends SlicerGraph<Integer> {
  ...
  public Set<Integer> getAllArgumentInstructionIndexes(int index)
  ...
  private static void addAllArgumentInstructionIndexes(
      int index,
      Graph<Integer, DefaultEdge> argumentDependencyGraph,
  ...
  public Set<Integer> getArgumentInstructionIndexes(int index)
      throws IOException, InvalidClassFileException {
    Graph<Integer, DefaultEdge> argumentDependencyGraph = getGraph();
    Set<DefaultEdge> outgoingEdges = argumentDependencyGraph.outgoingEdgesOf(index);
  ...
  public Map<Integer, Set<Integer>> getVarIndexesToRenumber()
      throws IOException, InvalidClassFileException {
    ...
    for (int index = 0; index < instructions.length - 1; index++) {
      ...
      if (instruction instanceof ILoadInstruction) {
        ...
        //				System.out.println("Search Store for Load with varIndex " + varIndex + " at " +
        // index);
        Set<Integer> directlyReachableVarInstructions = new HashSet<>();
        getNextVarInstructions(
            new HashSet<>(), directlyReachableVarInstructions, instructions, varIndex, index);
    ...
    //				IStoreInstruction storeInstruction = (IStoreInstruction)
    // instructions[anyInstructionIndex];
  ...
  private void getNextVarInstructions(
      Set<Integer> visited,
      Set<Integer> directlyReachableVarInstructions,
    ...
    for (DefaultEdge edge : cfg.incomingEdgesOf(index)) {
      ...
      getNextVarInstructions(
          visited, directlyReachableVarInstructions, instructions, varIndex, prevIndex);
    }
    for (DefaultEdge edge : cfg.outgoingEdgesOf(index)) {
      int nextIndex = cfg.getEdgeTarget(edge);
      getNextVarInstructions(
          visited, directlyReachableVarInstructions, instructions, varIndex, nextIndex);
    }
  }
