...
public abstract class SlicerGraph<T> {
	...
	protected GraphExporter<T, DefaultEdge> getExporter(ComponentNameProvider<T> vertexIdProvider,
			ComponentNameProvider<T> vertexLabelProvider) {
		DOTExporter<T, DefaultEdge> exporter = new DOTExporter<>(vertexIdProvider, vertexLabelProvider, null);
	...
	protected String getExporterGraphString(ComponentNameProvider<T> vertexIdProvider,
			ComponentNameProvider<T> vertexLabelProvider)
	...
	public void showPlot() throws IOException, InterruptedException, InvalidClassFileException, ExportException {
		final Path dir = Files.createTempDirectory("slicer-");
		showPlot(dir);
	}

	protected abstract String dotPrint() throws IOException, InvalidClassFileException, ExportException;
}
