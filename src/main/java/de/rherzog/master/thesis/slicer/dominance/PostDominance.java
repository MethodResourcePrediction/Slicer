...
public class PostDominance extends SlicerGraph<Integer> {
	...
	public PostDominance(ControlFlow controlFlow, int startIndex) throws IOException, InvalidClassFileException {
	...
	public Graph<Integer, DefaultEdge> getGraph() throws IOException, InvalidClassFileException {
		...
		for (int y : cfg.vertexSet()) {
			final List<GraphPath<Integer, DefaultEdge>> cfgPathFromYToEnd = cfgPaths.getAllPaths(Set.of(y), endNodes,
					true, cfg.vertexSet().size());
